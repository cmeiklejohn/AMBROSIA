...
//                    if (reconnecting) // BUGBUG We are temporarily disabling this optimization which avoids unnecessary locking as reconnecting is not a sufficient criteria: We found a case where input is arriving during reconnection where counting was getting disabled incorrectly. Further investigation is required.
    ...
    public class AmbrosiaRuntime : VertexBase
    {
        ...
        enum AARole { Primary, Secondary, Checkpointer, Reshard };
        AARole _myRole;
                                               ...
                                               bool testUpgrade = false)
        {
            ...
            if (!_runningRepro && (_createService || _oldShards.Length == 0))
            {
        ...
        private void RecoverFromShard(long shardID, long checkpointToLoad = -1, bool testUpgrade = false)
        {
            MachineState state = new MachineState();
        ...
        private void RecoverFromShards(long checkpointToLoad = -1, bool testUpgrade = false)
        {
            ...
            for (int i = 0; i < _oldShards.Length; i++)
            {
                ...
                threads[i] = new Thread(() => RecoverFromShard(shardID, checkpointToLoad, testUpgrade)) { IsBackground = true };
                threads[i].Start();
            }
        ...
        private void WaitForOtherShards()
        {
            ...
            if (index == -1)
            {
                throw new Exception("Shard ID not in list");
            }
            if (index > 0)
            {
                while (!File.Exists(ReshardFileName(_newShards[index - 1])))
                {
                    Thread.Yield();
                }
            }
        ...
        public void Initialize(object param, long[] oldShards, long shardID, Func<long, long> shardMap)
        {
